...
* Version 1.1.1 - Added test for above 255 addresses
************************************************************************/



//#include "GenericTypeDefs.h"
//#include "..\Include\DEE Emulation 8-bit\DEE Emulation 8-bit.h"
...
//    TBLPTR = (short long)&Emulation_Page [0];
    ...
    TBLRDPOSTINC                            
    _endasm
    statusCount.byte1.LB = TABLAT;    // EE Status  (8-bits)
    _asm
    TBLRDPOSTINC                           
    _endasm
    statusCount.byte1.HB = TABLAT;    // Not used but available for higher E/W counts
    _asm
    TBLRDPOSTINC                            
    _endasm
    statusCount.byte1.UB = TABLAT;    // Lower byte of  E/W count
    _asm
    TBLRD                           
    _endasm
    statusCount.byte1.MB = TABLAT;    // Upper byte of E/W count
    ...
    {
        statusCount.byte1.LB &= 0b11111011;
        SetPageExpiredPage(1);
    }

    TBLPTR = packedPageAddr;

    TABLAT = statusCount.byte1.LB;     // EE Status
    _asm
    TBLWTPOSTINC
    _endasm
    TABLAT = statusCount.byte1.HB;     // Not used but available for higher E/W counts
    _asm
    TBLWT
    _endasm
    EECON1 = PROGRAM_WORD;
    UnlockWrite();
    TBLPTR++;
    TABLAT = statusCount.byte1.UB;     // Lower byte of  E/W count
    _asm
    TBLWTPOSTINC
    _endasm
    TABLAT = statusCount.byte1.MB;     // Upper byte of E/W count
    _asm
    TBLWT
    _endasm
    EECON1 = PROGRAM_WORD;
    ...
    TBLRDPOSTDEC                            
    _endasm
    if(TABLAT != statusCount.byte1.MB)
    {
        SetPageWriteError(1);
        return(7);
    }   
    ...
    TBLRDPOSTDEC                            
    _endasm
    if(TABLAT != statusCount.byte1.UB)
    {
        SetPageWriteError(1);
        return(7);
    }
    ...
    TBLRDPOSTDEC                            
    _endasm
    if(TABLAT != statusCount.byte1.HB)
    {
        SetPageWriteError(1);
        return(7);
    }   
    ...
    TBLRD                            
    _endasm
    if(TABLAT != statusCount.byte1.LB)
    {
        SetPageWriteError(1);
        return(7);
    }
